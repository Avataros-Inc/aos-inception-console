...
const RenderJobVideo = ({ renderJobID }) => {
  ...
  const styles = {
    ...
    container: {
      ...
      overflow: 'hidden',
      backgroundColor: 'rgb(26, 33, 45)', // Using --bg-secondary
      border: '1px solid rgb(42, 47, 58)', // Using --border-subtle
    ...
    loading: {
      ...
      fontSize: '2rem',
      color: '#a1a1aa', // Using --text-secondary
    },
    error: {
      position: 'absolute',
      top: '50%',
      ...
      <div style={styles.container}>
        ...
        {error && (
          <div style={styles.error}>
            {error}
            {retryAfter > 0 && <div>Retrying in {retryAfter} seconds...</div>}
          </div>
        )}

        {!renderJobID && <div style={styles.error}>No render job ID provided</div>}
      </div>
    </div>
  );
};
